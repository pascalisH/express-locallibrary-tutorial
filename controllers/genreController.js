...
exports.genre_delete_get = function(req, res, next) {
  async.parallel({
    ...
    if (results.genre==null) { // No results.
     ...
     //Successful, so render.
    res.render('genre_delete', { title: 'Delete Genre', genre: results.genre, booksInGenre: results.books } );
});
  ...
  (req, res, next) => {
      ...
      if (!errors.isEmpty()) {
          ...
          .exec(function (err, genre) {
            ...
            res.render('genre_form', { title: 'Update Genre '+genre.name, genre: updatedGenre, errors: errors.array() });
            return;
          })
      }
      else {
          // Data from form is valid. Update the record.
          Genre.findByIdAndUpdate(req.params.id, updatedGenre, {}, function (err,thegenre) {
              if (err) { return next(err); }
